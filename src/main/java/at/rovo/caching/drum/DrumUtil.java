...
/**
 * <p>This utility class provides some basic methods to convert 32bit integer values 
 * to a byte-array and vice versa and to extract 64bit hash codes from String objects 
 * and get the number of a bucket based on a key and the number of buckets for 32- 
 * and 64bit calculations.</p>
 * <p>Note however, that Java only supports lists with 32bit integer length - therefore
 * the 64bit versions here aren't used in the application itself.</p>
 * 
...
{	
	/**
	 * <p>Calculates a 8-byte (64bit) hash from a {@link String}</p>
	 * 
	 * @param string
	 * @return
	 */
	...
	/**
	 * <p>Calculates a 8-byte (64bit) hash from an object</p>
	 * 
	...
	/**
	 * <p>Calculates the bucket id the key should be placed in according to the
	 * first n bits of the key where n is the power of 2 numBuckets is defined
	 * as. F.e if 1024 buckets are defined the first 10 bits of the key will
	 * be taken as the index for the actual bucket (2^10 = 1024, 2^9 = 512, ...)</p>
	 * 
	 * @param key The 64bit key whose bucket index should be calculated
	 * @param numBuckets The total number of available buckets. This should be
	 *                   a power of two (e.g. 2, 4, 8, 16, 32, ...)
	...
	/**
	 * <p>Calculates the bucket based on a 64bit key.</p>
	 * <p>The calculation is based on an example presented by
	 * Leandro T C Melo (http://www.codeproject.com/Articles/36221/DRUM-A-C-Implementation-for-the-URL-seen-Test-of-a)</p>
	 * 
	 * @param key The key whose bucket should be calculated
	 * @param numBuckets The total number of available buckets. This should
	 *                   be a power of two (e.g. 2, 4, 8, 16, 32, ...)
	...
	/**
	 * <p>Calculates the bucket based on a 32bit key.</p>
	 * <p>The calculation is based on an example presented by
	 * Leandro T C Melo (http://www.codeproject.com/Articles/36221/DRUM-A-C-Implementation-for-the-URL-seen-Test-of-a)</p>
	 * 
	 * @param key The key whose bucket should be calculated
	...
	/**
	 * <p>Serializes an object into a byte-array</p>
	 * 
	 * @param obj The object to convert into a binary array
	 * @return The bytes of the object or an empty byte array if the
    ...
    /**
     * <p>Turns a byte-array into an object</p>
     * 
...
//	    	ois.mark(0);
    		...
    		{
    			System.err.println("Error deserializing object of type "+type.getCanonicalName()+" for bytes: "+bytes+"! Reason: "+e.getLocalizedMessage());
    			throw e;
    		}
    	}
        return ret;
    ...
    /**
     * <p>Converts a long value into a 8 byte (64bit) long byte array.</p>
     * 
    ...
    /**
     * <p>Converts an integer value into a 4 byte (32bit) long byte
     * array.</p>
     * 
    ...
    /**
     * <p>Converts a 4 byte long byte array into an integer value</p>
     * 
    ...
    /**
     * <p>Converts a 8 byte long byte array into a long value</p>
     * 
