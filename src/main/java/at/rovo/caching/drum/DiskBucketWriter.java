...
/**
 * <p><code>DiskBucketWriter</code> is a consumer in the producer-consumer 
 * pattern and it takes data stored from the in-memory buffer and writes
 * it to the attached disk bucket file.</p>
 * <p>According to the paper 'IRLbot: Scaling to 6 Billion Pages and Beyond'
 * the data is separated into key/value and auxiliary data files which are
 * stored in the cache/'drumName' directory located inside the application
 * directory where <code>drumName</code> is the name of the current Drum 
 * instance.</p>
 * <p>The current implementation uses the blocking method <code>takeAll()</code>
	...
	implements IDiskWriter<V, A>
{
	 ...
	 * store. It needs to be informed if it should merge, which happens if
	 * the bytes written to the disk file exceeds certain limits **/
	...
	/**
	 * <p>Creates a new instance and instantiates required fields.</p>
	 * 
	 * @param drumName The name of the Drum instance
	 * @param bucketId The index of the bucket this writer will read data from 
	 *                 or write to
	 * @param bucketByteSize The size in bytes before a merge with the backing
	 *                       data store is invoked
	...
	public DiskBucketWriter(String drumName, int bucketId, int bucketByteSize, 
			IBroker<InMemoryData<V,A>,V,A> broker, IMerger<V,A> merger, DrumEventDispatcher eventDispatcher)
	{
	...
	/**
	 * <p>Creates the bucket files for key/value and auxiliary data.</p>
	 */
	private void initDiskFile()
	{
		...
		catch (Exception e) 
		{
			if (logger.isErrorEnabled())
				logger.error(this.drumName+" - Error creating bucket file!",e);
			e.printStackTrace();
	...
	{		
		while (!this.stopRequested)
		{
			try
			{
				if (this.lastState == null)
				{
					if (logger.isDebugEnabled())
						logger.debug("["+this.drumName+"] - ["+this.bucketId+"] - waiting for data");
					
					this.lastState = DiskWriterState.WAITING_ON_DATA;
					this.eventDispatcher.update(new DiskWriterStateUpdate(this.drumName, this.bucketId, DiskWriterState.WAITING_ON_DATA));
				}
				...
				this.lastState = null;
				this.eventDispatcher.update(new DiskWriterStateUpdate(this.drumName, this.bucketId, DiskWriterState.DATA_RECEIVED));
				
				if (logger.isDebugEnabled())
					logger.debug("["+this.drumName+"] - ["+this.bucketId+"] - received "+elementsToPersist.size()+" data elements");
			...
			{
				if (logger.isErrorEnabled())
					logger.error("["+this.drumName+"] - ["+this.bucketId+"] - got interrupted!");
				e.printStackTrace();
				this.eventDispatcher.update(new DiskWriterStateUpdate(this.drumName, this.bucketId, DiskWriterState.FINISHED_WITH_ERROR));
				Thread.currentThread().interrupt();
			}
	...
	/**
	 * <p>Closes a previously opened {@link RandomAccessFile} and frees resources
	 * held by the application.</p>
	 * 
	 * @param bucketFile The previously opened bucket file which needs to be closed
	 */
	private void closeFile(RandomAccessFile bucketFile)
	{
		try 
		{
			bucketFile.close();
		...
		{
			if (logger.isErrorEnabled())
				logger.error("["+this.drumName+"] - ["+this.bucketId+"] - Exception closing disk bucket!", e);
			e.printStackTrace();
		...
		{
			if (logger.isDebugEnabled())
				logger.debug("["+this.drumName+"] - ["+this.bucketId+"] - Closing file "+bucketFile);
		}
	}
	
	/**
	 * <p>Feeds the key/value and auxiliary bucket files with the data stored
	 * in memory buffers.</p>
	 * 
	 * @param inMemoryData The buffer which contains the data to persist to 
	 *                     disk
	 */
	private synchronized void feedBucket(List<InMemoryData<V,A>> inMemoryData, boolean forced)
	{
		try
		{
			this.eventDispatcher.update(new DiskWriterStateUpdate(this.drumName, this.bucketId, DiskWriterState.WAITING_ON_LOCK));
			this.lock.acquire();
			this.eventDispatcher.update(new DiskWriterStateUpdate(this.drumName, this.bucketId, DiskWriterState.WRITING));
			...
			for (InMemoryData<V,A> data : inMemoryData)
			{
				if (logger.isInfoEnabled())
					logger.info("["+this.drumName+"] - ["+this.bucketId+"] - feeding bucket with: "+data.getKey()+"; value: "+data.getValue());
				...
				{
					if (logger.isInfoEnabled())
						logger.info("["+this.drumName+"] - ["+this.bucketId+"] - wrote to kvBucket file - operation: '"+c+"' key: '"+data.getKey()+"', value.length: '"+byteValue.length+"' byteValue: '"+Arrays.toString(byteValue)+"' and value: '"+data.getValue()+"' - bytes written in total: "+(kvEndPos-kvStartPos));
				}
				else
				{
					if (logger.isInfoEnabled())
						logger.info("["+this.drumName+"] - ["+this.bucketId+"] - wrote to kvBucket file - operation: '"+c+"' key: '"+data.getKey()+"', value.length: '0' byteValue: 'null' and value: '"+data.getValue()+"' - bytes written in total: "+(kvEndPos-kvStartPos));
				}
				...
				{
					if (logger.isInfoEnabled())
						logger.info("["+this.drumName+"] - ["+this.bucketId+"] - wrote to auxBucket file - aux.length: '"+byteAux.length+"' byteAux: '"+Arrays.toString(byteAux)+"' and aux: '"+data.getAuxiliary()+"' - bytes written in total: "+(auxEndPos-auxStartPos));
				}
				else
				{
					if (logger.isInfoEnabled())
						logger.info("["+this.drumName+"] - ["+this.bucketId+"] - wrote to auxBucket file - aux.length: '0' byteAux: 'null' and aux: '"+data.getAuxiliary()+"' - bytes written in total: "+(auxEndPos-auxStartPos));
				}
			}
			...
			this.eventDispatcher.update(new DiskWriterEvent(this.drumName, this.bucketId, this.kvBytesWritten, this.auxBytesWritten));
			...
			if (!forced)
			{
				...
				{
					if (logger.isInfoEnabled())
						logger.info("["+this.drumName+"] - ["+this.bucketId+"] - requesting merge");
		...
		{
			if (logger.isErrorEnabled())
				logger.error("["+this.drumName+"] - ["+this.bucketId+"] - Error feeding bucket! Reason: "+e.getLocalizedMessage(), e);
			e.printStackTrace();
