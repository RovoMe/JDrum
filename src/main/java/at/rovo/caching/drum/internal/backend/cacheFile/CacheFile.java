...
/**
 * <p>
 * <em>CacheFile</em> is a key/value storage which enables updates of existing
 * key/value pairs.
 * </p>
 * <p>
 * This key/value store is tailored for the Disk Repository with Update
 * Management structure in that it enables a one-through pass processing of the
 * actual stored data. Therefore entries written to the store have to be
...
public class CacheFile<V extends ByteSerializer<V>, A extends ByteSerializer<A>>
{
	...
	/**
	 * <p>
	 * Resets the internal file pointer to the start of the file and clears
	 * cached data to simulate a new run-through.
	 * </p>
	 */
	public void reset()
	{
	...
	/**
	 * <p>
	 * Returns the entry starting at the current cursor position. If the cursor
	 * is in the middle of an entry it will not find any useful results! So make
	 * sure to set the cursor to the beginning of an entry first before invoking
	 * this method!
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Writes a new pair of key and value data into the cache file in a sorted
	 * order depending on the value of the key.
	 * </p>
	 * <p>
	 * If a key with the same value exists, it will be overwritten to update the
	 * new value for this key. Existing entries located after the data to write
	 * will be moved further backwards.
	 * </p>
	 * 
	 * @param data
	 ...
	 *            The data to write into the cache file
	 * @param append
	 *            Specifies if the data to write should be appended to an
			...
			NotAppendableException, DrumException
	{
		...
		try
		{
			...
			else
			{
				...
				long restLength = this.file.length()
				...
				long pos = Math.max(entryStartPosition + shiftBits
	 ...
	 * @param cacheEntry
	 *            Defines if the data written should be temporarily cached to
	 *            compare it in the next iteration with the next entry
	...
	/**
	 * <p>
	 * Shifts the remaining content of a file by <code>byte2write</code> to the
	 * right, creating a gap to write the new data into. With <code>
	 * segmentSize</code> the number of bytes read in one block can be defined
	 * to prevent {@link OutOfMemoryError}s due to filling up the heap-space of
	 * the current JVM instance.
	 * </p>
	 * 
	 * @param byte2write
	 *            The number of bytes to extend the current file
	 * @param segmentSize
	 *            The size of a segment to read in case the total file length
			...
			throws IOException, NotAppendableException
	{
		...
		if (append)
		{
			...
			if (byte2write != this.entrySize)
				this.file.setLength(this.file.length()
						+ (byte2write - this.entrySize));
	...
	/**
	 * <p>
	 * Feeds a provided {@link List} with keys currently contained in the cache
	 * file.
	 * </p>
	 * 
	 * @param keys
