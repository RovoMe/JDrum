...
/**
 * <p><code>DiskFileMerger</code> merges all disk files with the backing data
 * store and classifies stored data into {@link DrumResult#DUPLICATE_KEY} and
 * {@link DrumResult#UNIQUE_KEY}.</p>
 * <p>It therefore applies the 4 steps presented by Lee, Wang, Leonard and 
 * Loguinov in there paper 'IRLbot: Scaling to 6 Billion Pages and Beyond' to
 * achieve merging in a single one pass through disk strategy.</p>
 * <p>The execution of the merge happens, by default, in the {@link Thread} this 
 * instance was started with. The execution was requested by {@link #doMerge()}. 
 * On calling {@link #forceMerge()} the execution of the merge is done in
 * the callers thread context.</p>
 * 
	...
	implements IMerger<V,A>
{
	...
	/** The class object of the auxiliary type. Necessary to safely cast the generic 
	 * type to a concrete type**/
	 ...
	 */
	public DiskFileMerger(String drumName, int numBuckets, IDispatcher<V,A> dispatcher, Class<V> valueClass, Class<A> auxClass, DrumEventDispatcher eventDispatcher)
	{
		...
		while (!this.stopRequested)
		{
			try
			{
				...
				if (this.lastState == null)
				{
					...
					this.eventDispatcher.update(new MergerStateUpdate(this.drumName, MergerState.WAITING_ON_MERGE_REQUEST));
				}
				...
				if (this.mergeList.contains(Boolean.TRUE))
				{
					...
					if (logger.isDebugEnabled())
						logger.debug("["+this.drumName+"] - Notify received! Merging data");
	...
	/**
	 * <p>Extracts the data from the disk files which need to be merged into
	 * the backing data store.</p>
	 * <p>This method applies the 4 steps presented in the paper 'IRLbot: 
	 * Scaling to 6 Billion Pages and Beyond' to utilize a one pass through disk
	 * file strategy.</p>
	 */
	public synchronized void merge(boolean forced)
	{
		if (logger.isDebugEnabled())
			logger.debug("["+this.drumName+"] - merging disk files (forced = "+forced+")");
		...
		{								
			...
			try
			{
				this.eventDispatcher.update(new MergerStateUpdate(this.drumName, MergerState.WAITING_ON_LOCK, writer.getBucketId()));
				if (logger.isDebugEnabled())
					logger.debug("["+this.drumName+"] - ["+writer.getBucketId()+"] - available permits for diskWriter: "+writer.accessDiskFile().availablePermits());
				writer.accessDiskFile().acquire();
				
				this.eventDispatcher.update(new MergerStateUpdate(this.drumName, MergerState.MERGING, writer.getBucketId()));
				if (logger.isDebugEnabled())
					logger.debug("["+this.drumName+"] - ["+writer.getBucketId()+"] - got lock of disk writer");
				...
				if (this.readDataFromDiskFile(writer))
				{
					...
					// 2) backing data store is sequentially read in chunks of delta 
					//    bytes and compared with the keys in the sorted bucket buffer
					// ... combined into 3)
					// 3) those key/value pairs that require an update are merged with
					//    the contents of the disk cache and written to the data store
					this.compareDataWithDataStore(this.sortedMergeBuffer);
					// 4) after all unique keys are found the original order of the
					//    bucket buffer is restored and the auxiliary data file is 
					//    read in chunks of delta bytes to process auxiliary data of
					//    unique keys
					...
					if (logger.isDebugEnabled())
						logger.debug("["+this.drumName+"] - ["+writer.getBucketId()+"] - resetting disk file "+writer.getBucketId());
					...
					if (logger.isDebugEnabled())
						logger.debug("["+this.drumName+"] - ["+writer.getBucketId()+"] - nothing to merge");
			}
			catch (Exception e)
			{
				if (logger.isErrorEnabled())
					logger.error("["+this.drumName+"] Error merging disk bucket files with data storage! Reason: "+e.getLocalizedMessage(), e);
				e.printStackTrace();
			}
			finally
			{
	...
	/**
	 * <p>Reads key/value pairs from a disk file and stores them in <code>
	 * sortedMergeBuffer</code> as {@link InMemoryData} objects.</p>
	 * 
	 * @param writer The already locked writer instance we access its disk file 
	 * @throws InterruptedException
	 */
	private boolean readDataFromDiskFile(IDiskWriter<V,A> writer) throws InterruptedException
	{
		...
		if (logger.isDebugEnabled())
			logger.debug("["+this.drumName+"] - Reading data from disk file"); 
		try
		{			
			...
			if (logger.isDebugEnabled())
				logger.debug("["+this.drumName+"] - ["+writer.getBucketId()+"] - reading "+writtenBytes+" bytes from bucket file");
			...
			while (kvFile.getFilePointer() < writtenBytes)
			{
				...
				// Retrieve the key from the file - as it is 64-bit long, reading
				// long should work
...
//					V value = DrumUtil.deserialize(byteValue, this.valueClass);
					V value = this.valueClass.newInstance().readBytes(byteValue);
					data.setValue(value);
				}
				if (logger.isDebugEnabled())
					logger.debug("["+this.drumName+"] - ["+writer.getBucketId()+"] - read from bucket file - operation: '"+op+"', key: '"+key+"', value.length: '"+valueSize+"' valueBytes: '"+Arrays.toString(byteValue)+"'");
			}
		...
		{
			if (logger.isErrorEnabled())
				logger.error("["+this.drumName+"] - Error during reading key/values from bucket file! Reason: "+e.getLocalizedMessage(), e);
			e.printStackTrace();
			throw new DrumException("Error during reading key/values from bucket file! Reason: "+e.getLocalizedMessage(), e);
		}
	...
	/**
	 * <p>Sorts the merge buffer based on the key-value.</p>
	 */
	private void sortMergeBuffer()
	{
		if (logger.isDebugEnabled())
			logger.debug("["+this.drumName+"] - sorting merge buffer");
		Collections.sort(this.sortedMergeBuffer, new KeyComparator<InMemoryData<V,A>>());
	}
	
	/**
	 * <p>Checks the keys of data elements stored in the bucket buffer with
	 * keys already stored in the backing data store and merges them into the 
	 * data store if they need an update.</p>
	 * <p>If the key is already present in the data store the result field of 
	 * the data object will be set to {@link DrumResult#DUPLICATE_KEY}, else to 
	 * {@link DrumResult#UNIQUE_KEY}.</p>
	 * 
	...
	/**
	 * <p>Sets the cursor of the data store back to the start.</p>
	 */
	protected abstract void reset();
	
	/**
	 * <p>Reverts the origin order of the merge buffer. {@link #unsortingHelper} 
	 * afterwards contains the origin position of the merge buffer, while the 
	 * {@link #sortedMergeBuffer} is not touched!</p>
	 */
	private void unsortMergeBuffer()
	{
		if (logger.isDebugEnabled())
			logger.debug("["+this.drumName+"] - unsorting merge buffer");
		// When elements were read into the merge buffer their original positions
	...
	/**
	 * <p>Reads a certain bucket containing auxiliary data from disk into memory. 
	 * The data read is afterwards stored inside the {@link #unsortedAuxBuffer} 
	 * as an {@link Auxiliary} object</p>
	 * 
	 * @param writer The already locked writer instance we access its disk file 
	 */
	private void readAuxBucketForDispatching(IDiskWriter<V, A> writer)
	{
		...
		{
			if (logger.isDebugEnabled())
				logger.debug("["+this.drumName+"] - ["+writer.getBucketId()+"] - reading auxiliary bucket '"+writer.getAuxFileName()+"' for dispatching");
			int i = 0;
...
//					A aux = DrumUtil.deserialize(byteAux, this.auxClass);
					...
					if (logger.isDebugEnabled())
						logger.debug("["+this.drumName+"] - ["+writer.getBucketId()+"] - read aux data: "+aux);
				}
				else
				{
					if (logger.isDebugEnabled())
						logger.debug("["+this.drumName+"] - ["+writer.getBucketId()+"] - no data to read from auxiliary data bucket file");
		...
		{
			if (logger.isErrorEnabled())
				logger.error("["+this.drumName+"] - ["+writer.getBucketId()+"] - Could not read auxiliary bucket file! Reason: "+e.getLocalizedMessage(), e);
			e.printStackTrace();
	...
	/**
	 * <p>Dispatches {@link DrumResult}s to the caller.</p>
	 */
	private void dispatch()
	{
		...
		for (int i = 0; i < total; ++i)
		{
			...
			// now retrieving the key, the action, the result of the query and the 
			...
			if (logger.isDebugEnabled())
				logger.debug("["+this.drumName+"] - dispatching: op: "+data.getOperation()+"; "
					+data.getKey()+"; value: "+data.getValue()+"; aux: "
