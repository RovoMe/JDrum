...
/**
 * <p><code>CacheFileMerger</code> uses {@link CacheFile} to check keys for their
 * uniqueness and merges data that needs to be updated with the cache file.</p>
 * <p>On successfully extracting the unique or duplicate status of an entry,
 * the result will be injected into the data-object itself to avoid loosing
 * data informations.</p>
 *
	...
	extends DiskFileMerger<V, A>
{
	...
	/**
	 * <p>Creates a new instance and initializes required instance fields.</p>
	 * 
	 * @param drumName The name of the DRUM instance this cache is used for
	 * @param dispatcher The dispatching instance to send results to
	 */
	public CacheFileMerger(String drumName, int numBuckets, IDispatcher<V,A> dispatcher, Class<V> valueClass, Class<A> auxClass, DrumEventDispatcher eventDispatcher)
	{
	...
	/**
	 * <p>Initializes and creates if necessary the directories and files for the
	 * cache file.</p>
	 * <p>Note that the cache file will be placed inside a cache directory of
	 * the applications root-path in a further directory which is named after
	 * the DRUM instance it is created for. The actual cache file is named <code>
	 * cache.db</code>.</p>
	 */
	private void initCacheFile()
	{
		...
		this.cacheFile = new CacheFile<V>(drumDir+"/cache.db", this.drumName, this.valueClass);
	}
	...
	protected void compareDataWithDataStore(List<? extends InMemoryData<V, A>> data)
	{
		for (InMemoryData<V,A> element : data)
		{
			...
			if (DrumOperation.CHECK.equals(op) || DrumOperation.CHECK_UPDATE.equals(op))
			{
				...
				{
					logger.error("["+this.drumName+"] - Error retrieving data object for key: "+key+" ("+element+")! Reason: "+e.getLocalizedMessage(), e);
			...
			if (DrumOperation.UPDATE.equals(op) || DrumOperation.CHECK_UPDATE.equals(op))
			{
				...
				{
					logger.error("["+this.drumName+"] - Error writing data object for key: "+key+" ("+element+")! Reason: "+e.getLocalizedMessage(), e);
	...
	/**
	 * <p>Returns a reference to the cache file used to store the data.</p>
	 * 
